...
[ W1, D1, n_ev, it, itv, flag ] = subspace_iter_v1(A, p, percentage,  eps, maxit);

toc

D1 = diag(D1);
[D1, indice] = sort(D1, 'descend');
D1 = diag(D1);
W1 = W1(:, indice);
...
for k = inter
    if (p < q)
        Im_k = W2(:, 1:k) * Vsing(1:k, 1:k) * W1(:, 1:k)';
    else
        Im_k = W1(:, 1:k) * Vsing(1:k, 1:k) * W2(:, 1:k)';
    end
