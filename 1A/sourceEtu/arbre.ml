...
let rec retrait_arbre lc (Noeud (b, lb)) =
   ...
   | c::qlc -> let arbre_c =
    ...
    let arbre_sujet1 =
      List.fold_right retrait_arbre [['b';'a';'s']] arbre_sujet

      (* Pour les tests *)
let bb = ('b',Noeud(false,[('a',Noeud(false,[('s',Noeud(false,[]));('t',Noeud(true,[]))]))]))
let bd = ('d',Noeud(false,[('e',Noeud(true,[]))]))
let bl = ('l',Noeud(false,[('a',Noeud(true,[('i',Noeud(true,[('d',Noeud(true,[]));('t',Noeud(true,[]))]));('r',Noeud(false,[('d',Noeud(true,[]))]))]));
                           ('e',Noeud(true,[('s',Noeud(true,[]))]));
                           ('o',Noeud(false,[('n',Noeud(false,[('g',Noeud(true,[]))]))]))]))
let b1l = [bb;bd;bl]
(* let arbre_sujetr = Noeudlet%test _ = retrait "bas" trie_sujet 
 *)   let arbre_sujet2 =
...
let rec parcours (Noeud (b, lb)) =
   match lb with
   | [] -> if b then [[]] else []
   | t::q -> let (t1, t2) = t in let Noeud(b1, _) = t2 in 
   if b1 then ([]::(List.map (fun l -> t1::l) (parcours t2))) 
      @ (parcours (Noeud(b, q)))
      else (List.map (fun l -> t1::l) (parcours t2)) @ (parcours (Noeud(b, q)))
...
(*utop # parcours arbre_sujet;;
- : char list list =
[['b'; 'a']; ['b'; 'a'; 's']; ['b'; 'a']; ['b'; 'a'; 't']; ['d']; ['d'; 'e'];
 ['l']; ['l'; 'a']; ['l'; 'a'; 'i']; ['l'; 'a'; 'i'; 'd']; ['l'; 'a'; 'i'];
 ['l'; 'a'; 'i'; 't']; ['l'; 'a'; 'i']; ['l'; 'a'; 'r']; ['l'; 'a'; 'r'; 'd'];
 ['l'; 'a']; ['l']; ['l'; 'e']; ['l'; 'e'; 's']; ['l'; 'e']; ['l'; 'o'; 'n'];
 ['l'; 'o'; 'n'; 'g']]*)



 (* List.fold_right (fun (c, sous_arb) qt -> let norm_abr = normalisation sous_arb in if norm_abr = Noeud(false,[]) then qt else (c, norm_abr)::qt) lb [] *)
let rec Normalise (Noeud (b, lb)) =
   if lb = [] then Noeud (b, lb)
      let lbNorm = 
         List.fold_right (fun (c, sArb) qlb -> let normArb = normalisation sArb in  if normArb = Noeud(false,[]) then qlb else (c, normArb)::qlb) lb [] in
  ...
  (*  else 
      (List.fold_right (fun t q -> let t = (c, a) in if a = Noeud(false,[]) then q  else t::q ) 
      (List.map ( fun t -> let t = (c, a) in (c, Normalise t))) []) *)
...
(* Avec match au lieu de fold_right: *)
...
let rec normaliser (Noeud (b, lb)) =
   if lb = [] then Noeud(b, lb) 
      ...
      let lbNorm = 
         match lb with
         | [] -> []
         | (c, sArb)::qlb  -> let normArb = normalisation sArb in if normArb = Noeud(false,[]) then qlb else (c, normArb)::qlb
      in if (not b) && (lbNorm = []) then 
         Noeud(false,[])
      else
         Noeud(b, lbNorm) 
*)