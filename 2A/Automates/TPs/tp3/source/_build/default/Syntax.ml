  ...
  *)
  ...
  (match (peekAtFirstToken stream) with
    ...
    (* | FunctionToken -> (match (accept FunctionToken stream) with
      | (Success next) -> (match (peekAtFirstToken next) with
        | IdentToken _ -> (match (accept (IdentToken _) next) with
          | (Success next1) -> (match (peekAtFirstToken next1) with
            | BodyToken -> (match (accept BodyToken next1) with
              | (Success next2) -> parseE next2
              | _ -> Failure)
            | _ -> Failure) 
          | _ -> Failure)
        | _ -> Failure) 
    ...
    | IfToken -> (match (accept IfToken stream) with
      | (Success next) -> (match (parseE next) with
        | (Success next1) -> (match (peekAtFirstToken next1) with
          | ThenToken -> (match (accept ThenToken next1) with
            | (Success next2) -> (match (parseE next2) with
              | (Success next3) -> (match (peekAtFirstToken next3) with
                | ElseToken -> (match (accept ElseToken next3) with
                  | (Success next4) -> parseE next4
                  | _ -> Failure)
                | _ -> Failure) 
              | _ -> Failure)
            | _ -> Failure)
...
and parseFX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    | TimesToken -> (match (accept TimesToken stream) with
      | (Success next1) -> (match (parseF next1) with
