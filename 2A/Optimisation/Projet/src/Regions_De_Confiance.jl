...
function regions_de_confiance(f::Function, gradf::Function, hessf::Function, x0::Vector{<:Real};
    ...
    Δ0::Real=2, Δmax::Real=10, γ1::Real=0.5, γ2::Real=2, η1::Real=0.25, η2::Real=0.75, algo_pas::String,
    ...
    if (norm(gradf(x0)) <= tol_abs)
        ...
        while i <= max_iter
            ...
            xk = xkp1 
            if algo_pas == "gct"
                # Calcul du pas a travers l'algo du gradient conjugué tronqué
                sk = gct(gradf(xk),hessf(xk),delta)
            elseif algo_pas == "cauchy"
                # Calcul du pas de cauchy
                sk = cauchy(gradf(xk),hessf(xk),delta)
            end
            
            # Calcul du module quadratique
            q(s) = f(xk) + gradf(xk)' * (s - xk) + (1/2) * (s - xk)' * hessf(xk) * (s - xk)
            ek = (f(xk) - f(xk + sk))/(q(xk) - q(sk + xk))
