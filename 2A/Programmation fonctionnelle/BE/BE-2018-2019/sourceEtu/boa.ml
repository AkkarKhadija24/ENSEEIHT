...
struct
  ...
  let construit_arbre lc n = 
    let id = Regle.id
    let rec aux p = 
      match p with
      | 0 -> []
      | _ -> (List.map (fun l -> (id, ArbreReecritureBOA.creer_noeud l 
      (construit_arbre l p))) (Regle.appliquer lc)) @ (aux (p-1))
    in ArbreReecritureBOA.creer_noeud lc (aux n)
