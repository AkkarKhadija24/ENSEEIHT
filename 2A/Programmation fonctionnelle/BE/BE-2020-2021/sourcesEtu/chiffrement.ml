...
struct
 ...
 let arbre4 =
  ...
  let get_branche e arb =
    match arb with
    | Lettre _ -> None
    | Noeud(l) -> let sous_arbre = List.assoc e l in Some sous_arbre
 ...
 let%test _ = get_branche 1 arbre1 = Some (Noeud ([(1,Lettre 'e');(2,Lettre 'b')]))
  ...
  let rec dechiffrer  lInt arb =
    let rec aux l a =
      match l with
      | [] -> []
      | t::q -> (match (get_branche t a) with
                | None -> raise CodeNonValide
                | Some aa -> (match aa with
                              | Lettre c -> c::dechiffrer q a
                              | _ -> aux q aa))
    in (aux lInt arb)
 ...
 (* let%test _ = dechiffrer [1;2;3;2;1;2] arbre1 = ['b';'a';'c']
  ...
  let  arbre_to_liste  arb = 
      match arb with
      | Lettre c -> [(c, [1])]
      | Noeud (l) -> 
      (List.map (fun (c, path) -> (c, 1::path)) (arbre_to_liste l)) 
      | _ -> []
 ...
 *)
  ...
  let rec chiffrer  arbre  = 
    let listea = arbre_to_liste arbre in
      match ld with 
      | [] -> []
      | t::q -> (List.assoc t listea)::(chiffrer q arbre)
