...
let rec height tr = 
    match tr with
    | Leaf _ -> 0
    | Node (trg, trd) -> max ((height trg) + 1) ((height trd) + 1)
 ...
 *)
 ...
 let rec num_nodes tr = 
    match tr with
    | Leaf _ -> 1
    | Node (trg, trd) -> (num_nodes trg) + (num_nodes trd) + 1
 ...
 *)
 let rec num_values tr =
    match tr with
    | Leaf c -> if c = _0 then 0 else 1
    | Node (trg, trd) -> (num_values trg) + (num_values trd)
 ...
 *)
 let rec search tr addrl = 
    ...
    | Leaf c, [] -> c 
    | Leaf _0, [_] -> _0
    | Node (trg, trd), t::q -> if t = 0 then search trg q 
    else search trd q
    | _ -> failwith ""
...
let sprout x addrl = 
List.fold_right (fun t sprtTrie -> if t = 1 then Node((Leaf _0), sprtTrie)
else Node(sprtTrie, (Leaf _0))) addrl (Leaf x)
 ...
 *)
 let rec update tr x addrl = 
    ...
    | Leaf _, [] -> Leaf x
    | Node (_, _), [] -> failwith ""
    | Leaf _, t::q -> if t = 0 then Node ((sprout x q), Leaf _0) else 
        Node (Leaf _0, sprout x q)
    | Node (trg, trd), t::q -> if t = 0 then  Node(update trg x q, trd) 
        else Node (trg, update trd x q)
 let%test "update-0" =
