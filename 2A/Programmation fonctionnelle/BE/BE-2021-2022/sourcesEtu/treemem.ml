...
struct
     ...
     *)
    let allocsize (_, tr) = height tr * bs

    (* Occupation de l'arbre. On ne compte que les feuilles non égales à 0. *)
    let busyness (bs, tr) = num_values tr

    (* La mémoire vide est celle qui ne contient aucun chemin *)
    let clear bs = (bs, sprout _0 (bits bs bs))

    (* Implémentation de read. *)
    let read (bs, tr) addr = if addr < size (bs, tr) then 
        search tr (bits bs addr) else raise OutOfBound
        

    (* Implémentation de write. *)
    let write (bs, tr) addr value = 
        if busyness (bs, tr) + addr < size (bs, tr) then (bs , update tr value (bits bs addr))
