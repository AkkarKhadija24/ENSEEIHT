...
struct
  ...
  let rec decompose e =    
    if e = baseTravail then [ 1; 0 ]
    else if e <= 0 then []
    else if e < baseTravail then [e]
    else (decompose (e / baseTravail))@[e mod B.base]
  ...
  let rec recompose l =
    match l with
    | [] -> 0
    | t::q -> t  * int_of_float ( float B.base ** float (List.length q)) + recompose q
