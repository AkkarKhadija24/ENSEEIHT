...
struct
  ...
  let rec conforme arb = 
    match arb with
    | Vide -> true
    | Noeud (_, l) -> if (List.length l <> B.base) then false 
    else List.for_all conforme l
  ...
  (* L'ensemble de départ et l'ensemble d'arrivée sont conformes à la base *)
   let rec appartient l arb = 
    match l, arb with
    | [], Noeud (true, _) -> true
    | [], _ -> false
    | _, Vide -> false
    | hd::_, Noeud (true, _) when hd = 0 -> false
    | hd::tl, Noeud (_, l) when hd >= 0 && hd < B.base -> 
      (let sous_l = List.nth_opt l hd in 
      match sous_l with
      | Some a -> appartient tl a
      | None -> false)
    | _, _ -> false
    ...
    (* L'ensemble de départ et l'ensemble d'arrivée sont conformes à la base *)
     (* let rec ajouter l arb =
        match (l, arb) with 
        | [], Noeud (b, ll) when b -> arb 
        | [], _ -> Noeud (true, [])  
        | hd::tl, Noeud (b, ll) when hd >= 0 && hd < B.base ->
            let enfant = List.nth_opt ll hd in
            let newLl =
              match enfant with
              | Some enfant -> ajouter tl enfant
              | None -> ajouter tl Vide 
            in
            let newLls = 
              let rec aux i acc ll =
                match i, ll with
                | 0, _ -> List.rev_append acc (newLl :: ll)
                | _, t::q -> aux (i-1) (t :: acc) q 
                | _, [] -> List.rev_append acc [newLl]  
              in
              aux hd [] ll
            in
            Noeud (b, newLls)
