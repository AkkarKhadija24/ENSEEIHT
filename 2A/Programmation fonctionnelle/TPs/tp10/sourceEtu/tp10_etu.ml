...
and parse_I : (char, inst) parser = fun flux ->
  (
    ((return []) ++ (parse_C >>= (fun cmd -> p_ptvirg >>= (fun _ -> parse_I >>= (fun i -> return (cmd::i))))))
  ) flux
...
let run_logo prog =
  ...
  let main () =
    try
      let file_name = "tp10.txt" in (* Remplacez par le nom de votre fichier LOGO *)
      let flux = flux_of_file file_name in
      let progs = parse_logo flux in
      match Solution.uncons progs with
    ...
    with
    | exn -> Format.printf "An error occurred: %s@." (Printexc.to_string exn)
