...
let compare_par_nombre (_, _, _, count1) (_, _, _, count2) =
  count2 - count1 > 0
;;

(* Tri de listStat avec ma fonction de tri fusion *)

let listStat_triee_fusion = tri_fusion compare_par_nombre listStat

(* Tri de listStat avec la fonction List.sort *)
let listStat_triee_builtin = List.sort (fun (_, _, _, count1) (_, _, _, count2) -> compare count2 count1) listStat
