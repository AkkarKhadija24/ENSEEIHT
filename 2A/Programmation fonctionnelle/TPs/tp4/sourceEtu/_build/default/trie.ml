...
(******************************************************************************)
let appartient mot (Trie(trie, decompose, recompose)) = 
  let decomposeMot = decompose mot in 
  appartient_arbre decomposeMot trie
...
(*   résultat   : aucun                                                       *)
(******************************************************************************)
let affiche p (Trie(arbre, fd, fr)) = 
  let l_arbre = (List.map fr (parcours_arbre arbre)) in 
  let rec draw l = 
    match l with
    | [] -> ()
    | t::q -> p t; draw q
...
let%test _ = not (appartient "bat"  trie_sujet)
