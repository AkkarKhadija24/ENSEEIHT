...
module Exo1 (ND : MONADE_PLUS) =
  struct
    ...
    let rec combinaisons k l = 
      ...
      | [], _ -> ND.zero
      | _, 0 -> ND.return []
      | t::q, _ -> ND.(combinaisons k q ++ (combinaisons (k-1) q >>= fun combinaison -> return (t::combinaison)))
...
module NDIter : MONADE_PLUS_PRINT =
  struct
    ...
    let uncons (Tick iter) = Lazy.force iter
    let zero = Tick (lazy None)
    let return iter = Tick (lazy (Some (iter, zero)))
    let rec ( ++ ) iter1 iter2 = Tick (lazy (
      match uncons iter1 with
      | None -> uncons iter2
      | Some (t, q) -> Some (t, q ++ iter2)
    ))

    let rec ( >>= ) iter f = Tick (lazy (
      match uncons iter with
      | None -> None
      | Some (t, q) -> uncons (f t ++ (q >>= f))
    ))

    let rec map f iter = Tick (lazy (
      match uncons iter with
      | None -> None
