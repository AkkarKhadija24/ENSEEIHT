...
public class FieldAccess extends AbstractField implements Expression {
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		while (i < this.recordType.getAllFields().size()) {
            if (this.recordType.getAllFields().get(i).getName().equals(name)) {
                _keep = this.recordType.getAllFields().get(i).getType().length();
                ++ i;
                break;
            }
            _removeBefore += this.recordType.getAllFields().get(i).getType().length();
            ++ i;
        }
        
        while (i < this.recordType.getAllFields().size()) {
            _removeAfter += this.recordType.getAllFields().get(i).getType().length();
            ++ i;
        }
