...
public class ArrayAllocation implements Expression {
	...
	public boolean collectAndBackwardResolve(HierarchicalScope<Declaration> _scope) {
		...
		return this.element.resolve(_scope) && this.size.collectAndBackwardResolve(_scope);
	}
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		result.append(this.size.getCode(_factory));
		result.add(_factory.createLoadL(this.element.length()));
		result.add(TAMFactory.createBinaryOperator(BinaryOperator.Multiply));
		// Ajout de createMalloc() dans TAMFactory (ast.tam)
		result.add(TAMFactory.createMalloc());
		return result;
	}
