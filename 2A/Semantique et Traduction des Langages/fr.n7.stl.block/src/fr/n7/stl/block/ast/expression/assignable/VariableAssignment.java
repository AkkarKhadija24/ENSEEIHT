...
public class VariableAssignment extends AbstractIdentifier implements AssignableExpression {
	...
	public boolean fullResolve(HierarchicalScope<Declaration> _scope) {
		if (_scope.knows(this.name)) {
			System.out.println("**** scope: " + _scope.get(name));
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		Register _register = this.declaration.getRegister();
		int _offset = this.declaration.getOffset();
		int _size = this.getType().length();
		resultFragment.add(_factory.createStore(_register, _offset, _size));
