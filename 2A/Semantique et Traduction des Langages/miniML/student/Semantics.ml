...
ruleCallByName env fexpr pexpr mem =
let (e1, m1) = (value_of_expr (fexpr, mem) env) in
  match e1 with
  | (FrozenValue (fexpr,fenv)) ->
    (match fexpr with
    | (FunctionNode (fpar,fbody)) ->
      (value_of_expr (fbody,m1) ((fpar,(FrozenValue (pexpr,env)))::fenv))
...
ruleRead _env _expr mem =  
  let (ve, vm) = (value_of_expr (_expr, mem) _env) in
  
  match ve with
  | (ErrorValue _) as result -> (result, vm)
  | (ReferenceValue _ref) -> (match lookforMem _ref vm with
      | NotFound -> ((ErrorValue (UnknownReferenceError _ref)), vm)
      | (Found value) -> (value, vm))
        
  | _-> ((ErrorValue TypeMismatchError), vm)
...
(* .............................................................................*)
 ruleWrite _env _refexpr _valexpr mem =
    let (v, mem2) = value_of_expr (_refexpr, mem) _env in
    match v with
    | ErrorValue _ as result -> (result, mem2)
    | ReferenceValue _ref ->
        let (v1, mem3) = value_of_expr (_valexpr, mem2) _env in
        (match v1 with
        | ErrorValue _ as result -> (result, mem3)
        | _ -> (NullValue, (_ref, v1) :: mem3))
    | _ -> 
          (ErrorValue TypeMismatchError, mem2)
...
ruleSequence _env _left _right mem = 
 let (v1, m1) = (value_of_expr (_left, mem) _env) in
  match v1 with
  | (ErrorValue _) as result -> (result, m1)
  | _ -> 
    let (v2, m2) = (value_of_expr (_right, m1) _env) in
    match v2 with
...
ruleWhile _env _cond _body mem = 
 let (value, mem2) = value_of_expr (_cond, mem) _env in
  match value with
  | (ErrorValue _) as result -> (result, mem2)
  | (BooleanValue false) -> (NullValue, mem2)
  | (BooleanValue true) -> 
    (let (_, mem3) = value_of_expr (_body, mem2) _env in
      ruleWhile _env _cond _body mem3 
    )
  | _ -> ((ErrorValue TypeMismatchError), mem2)
...
let (value, mem2) = (value_of_expr (_expr, mem) _env) in
match value with 
| (ErrorValue _) as result -> (result, mem2)
| _ -> 
  let ref = newReference () in
  ((ReferenceValue ref), (List.append mem2 [(ref, value)]))
;;
