...
let rec type_of_expr expr env =
  ...
  and
    ...
    let param_type = newVariable () in
    let body_type = type_of_expr _body ((_par, param_type) :: _env) in
    FunctionType(param_type, body_type)


  and
    (* ...............A COMPLETER .......................................*)
    ruleCall _env _fct _par = (* ErrorType *)
    let fct_type = type_of_expr _fct _env in
    (*Printf.printf "fct_type : %s\n" (string_of_type fct_type);*)
    let par_type = type_of_expr _par _env in
    (*Printf.printf "par_type : %s\n" (string_of_type par_type); *)
    let varEntree = newVariable() in 
    (*Printf.printf "varEntree : %s\n" (string_of_type varEntree);*)
    let varsortie = newVariable() in
    (*Printf.printf "varsortie : %s\n" (string_of_type varsortie);*)
    let _,uf = unify fct_type (FunctionType(varEntree, varsortie)) in
    let _,up = unify par_type varEntree in
    (*Printf.printf "varsortie2 : %s\n" (string_of_type varsortie);*)
    (if (uf && up) then varsortie else ErrorType)
  ...
  and
    ...
    | ReferenceType(referenced_type) ->
        let _, unified = unify referenced_type right_type in
        if unified then UnitType else ErrorType
    | _ -> ErrorType
  ...
  and
    ...
    ruleSequence _env _left _right = (* ErrorType *)
      let right_type = (type_of_expr _right _env) in
      let left_type = (type_of_expr _left _env) in
      let var = newVariable () in    
      let (_, bl) = (unify left_type UnitType) in
      let (tr, br) = (unify right_type var) in
      (* Printf.printf "Value of _left: %s\n" (string_of_type left_type);
      Printf.printf "Value of _right: %s\n" (string_of_type right_type);
      Printf.printf "tr : %s\n" (string_of_type tr); *)
      (if (bl && br) then tr else ErrorType)

  and
    (* ...............A COMPLETER .......................................*)
    ruleWhile _env _cond _body = (* ErrorType *)
      let cond_type = (type_of_expr _cond _env) in
      let body_type = (type_of_expr _body _env) in
      let _, unified = (unify cond_type BooleanType) in
      let _, ur = (unify body_type UnitType) in
      (if (unified && ur) then UnitType else ErrorType)
