    ...
    class Scheduler implements Runnable {
        public void run() {
            ...
            /* XXXX TODO: initialiser gchan. */
             for (int i = 0; i < nbPhilosophes; i++) {
                gchan[2 * i] = new GuardedChannel<>(entrer[i], () -> canEnter(i));
                gchan[2 * i + 1] = new GuardedChannel<>(sortir[i], Predicate::True);
            }

            /* XXXX TODO: construire l'alternative. */
            Alternative alt = new Alternative(gchan);

            /* XXXX TODO: boucle avec select et traitement selon l'id du canal où une lecture est possible. */
             while (true) {
                int index = alt.select();
                int philosopher = index / 2;

                if (index % 2 == 0) {
                    // Entrer
                    IHMPhilo.poser(Main.EtatPhilosophe(philosopher), EtatPhilosophe.Demandeur);
                    entrerFourchettes(philosopher);
                } else {
                    // Sortir
                    IHMPhilo.poser(Main.EtatPhilosophe(philosopher), EtatPhilosophe.Penseur);
                    libererFourchettes(philosopher);
                }
            }

            private boolean canEnter(int philosopher) {
            // Vérifier si aucun des voisins ne mange
                int leftNeighbor = (philosopher + nbPhilosophes - 1) % nbPhilosophes;
                int rightNeighbor = (philosopher + 1) % nbPhilosophes;

                return IHMPhilo.EtatPhilosophe(leftNeighbor) != EtatPhilosophe.Mangeur
                        && IHMPhilo.EtatPhilosophe(rightNeighbor) != EtatPhilosophe.Mangeur;
            }
        }
    } // class Scheduler
