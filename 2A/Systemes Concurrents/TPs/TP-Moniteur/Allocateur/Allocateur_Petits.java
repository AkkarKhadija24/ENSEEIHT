...
public class Allocateur_Petits implements Allocateur {
    ...
    public Allocateur_Petits(int nbRessources) {
        ...
        for (int i = 1; i <= nbRessources; i++) {
            this.classe[i] = this.moniteur.newCondition();
        }
    }
    ...
    public void liberer(int rendu) throws InterruptedException{
        ...
        // Débloquer en commençant par considérer les plus petits demandeurs
        int i = 1;
        while (i <= rendu && tailleClasse[i] > 0) {
            // Libérer une ressource pour chaque processus en attente à cet étage
            nbLibres--;
            tailleClasse[i]--;
            classe[i].signal();
            i++;
